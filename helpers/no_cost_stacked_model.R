...
trainTask = makeClassifTask(data = tr, target = "return")
...
predictions_known = predict(stack_model, newdata = df_known)
predictions_class = predict(stack_model, newdata = df_class)

# assess performance
d.result = data.frame(d$order_item_id, ifelse(predictions_known$data$response == "return0",0,1))
...
classdata.result = data.frame(df_class$order_item_id, ifelse(predictions_class$data$response == "return0",0,1))
