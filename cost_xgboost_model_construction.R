...
cost = data.frame(ifelse(d$return == 0, 0, 0.5*(3+0.1*d$item_price)),
...
trainTask = makeCostSensTask(id = "return",
                             data = tr,
                             cost = cost)
...
xgb_learner = makeCostSensClassifWrapper(xgb_learner)
...
predicted_classes = predict(xgb_model, newdata = ts)
predicted_class   = predict(xgb_model, newdata = classdata)

d.result = data.frame(d[-idx.train,]$order_item_id, ifelse(predicted_classes$data$response == "X0",0,1))
names(d.result) = c("order_item_id", "return")
accuracy = mean(d.result$return == ts$return)

classdata.result = data.frame(classdata$order_item_id, ifelse(predicted_class$data$response == "X0",0,1))
